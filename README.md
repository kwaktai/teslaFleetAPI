...
차량별로 아래 버튼이 나옵니다. 휴대폰 홈 화면에 추가해 두면 앱처럼 쓸 수 있습니다.
문 열기는 확인창을 한 번 거칩니다.

깨우기 · 문 열기 · 문 잠금 · 공조 켜기/끄기 · 전조등 · 경적 ·
액세서리 전원 유지/해제 · 저전력 모드 켜기/끄기

`enable` 같은 값을 받는 토글 명령은 켜기·끄기를 각각 버튼으로 두고 해당 JSON 본문을 보냅니다.
...
**81개 명령**과 각 명령의 파라미터를 문서 페이지에서 확인할 수 있습니다. 검색창으로 걸러볼 수 있습니다.

```
https://<내도메인>/document
```

명령은 전달 경로가 두 가지이며, 문서에 배지로 표시됩니다.

| 배지 | 개수 | 전달 경로 |
|---|---|---|
| **서명** | 69 | 서명 프록시가 개인키로 서명해 전송. 2021년 이후 차량의 정식 경로 |
| **직접** | 12 | 프록시가 구현하지 않은 명령이라 Fleet API 로 서명 없이 전달. 차량이 서명을 요구하면 Tesla가 거부할 수 있음 |

"서명" 목록은 `vehicle-command v0.4.1` 소스에서 추출했으므로 파라미터 이름과 필수 여부가
프록시의 실제 동작과 일치합니다. 분류는 도어·트렁크·창문(9), 공조·시트(12), 충전(15),
보안·접근 제한(12), 미디어(9), 기타·차량 관리(12), 프록시 미지원(12) 입니다.

> 프록시는 모르는 명령을 받으면 `invalid_command`(400)로 거부하고 전달하지 않습니다.
> 그래서 "직접" 명령은 서버가 프록시를 건너뛰고 Fleet API 로 보냅니다.
...
| 기능 | 연결 전 | 연결 후 |
|---|---|---|
| 차량 목록 (`/api/vehicles`) | Fleet (과금) | Owner (무료) |
| 차량 상태 조회 | Fleet (과금) | Owner (무료) |
| 깨우기 · 온라인 폴링 | Fleet (과금) | Owner (무료) |
| 명령 (문 잠금 등) | Fleet | Fleet (서명 필요, 변경 없음) |

연결하면 **Fleet API 로 나가는 것은 명령뿐**입니다.
GET 요청도 "데이터" 범주로 과금되므로, 제어 페이지를 열 때마다 호출되는
차량 목록까지 Owner 로 돌립니다.
...
`invalid_auth_code` 가 나오면 code 가 만료되었거나 이미 사용된 것입니다.
페이지의 **↻ 새 로그인 세션으로 다시 시작** 을 눌러 새 주소를 받은 뒤 다시 로그인하세요.
code 는 발급 당시의 `code_verifier` 와만 짝이 맞으므로, 오래된 창에 떠 있는 주소를
재사용하면 실패합니다.
...
상태 페이지(`https://<내도메인>/`)에 **이번 달 API 사용량** 표가 있습니다.
Fleet 로 나간 요청 수와 예상 요금, Owner 로 빠져 절약한 금액을 함께 보여줍니다.

```
GET  /api/usage         지난달 이전 기록까지 JSON 으로
POST /api/usage/reset   기록 초기화
```

### 단가

개발자 포털 → **청구 및 사용량** 의 "범주별 비용" 에서 역산한 값입니다.

| 종류 | 건당 | 비고 |
|---|---|---|
| 명령 | ₩1.4 | 문 잠금·공조 등 |
| 조회 | ₩2.7 | `vehicle_data` 등 |
| **깨우기** | **₩25** | **명령의 약 18배 — 가장 비쌉니다** |

깨우기가 압도적으로 비싸므로, Owner API 연결의 절감 효과 대부분은 여기서 나옵니다.
단가가 바뀌면 `.env` 의 `PRICE_COMMAND` / `PRICE_DATA` / `PRICE_WAKE` 로 조정하세요.

> 서버가 직접 센 값이라 Tesla 청구서와 완전히 같지는 않을 수 있습니다.
> 정확한 청구액은 [개발자 포털](https://developer.tesla.com)에서 확인하세요.
