...
시리얼 115200 에서 `CAN A MCP2518FD listen-only 500k OK (X437 9/10) xtal=40.0MHz(lib 40)` 가 보이면 됩니다. `xtal=` 은 부팅 때 MCP2518FD 카운터로 실측한 크리스털 주파수와 라이브러리에 넘긴 클럭입니다 — 아래 "CAN A 가 조용할 때" 참고.
...
| `신호는 있는데 프레임 오류` | stuff/form/crc 에러가 늘어남 | 속도 불일치(`CANA SCAN`), 크리스털 클럭(아래 `xtal=`), H/L 바뀜 |
...
**크리스털 40MHz 문제 (2026-09-11 수정).** 차에서 A 가 `신호는 있는데 프레임 오류`(stuff/form/bit1, `frames=0`) 만 찍힌 원인. Longan_CANFD 라이브러리는 `begin(속도)` 에 클럭을 안 주면 MCP2518FD 가 **20MHz** 로 돈다고 보고 비트타이밍을 계산하는데, T-2CAN FD 회로도(`T-2Can-Fd_V1.0.pdf`)의 MCP2518FD 크리스털은 **X1 40MHz** 입니다. 그래서 "500k" 로 설정하면 실제 버스 속도는 **1Mbps** 가 되어 500k 프레임이 전부 에러로 버려졌습니다. H/L 을 바꿔 꽂아도 똑같이 보이는 증상입니다. 이제 부팅 때 칩의 Time Base Counter 로 SYSCLK 을 200ms 실측해(`xtal=40.0MHz(lib 40)`) 맞는 클럭 상수를 라이브러리에 넘깁니다. `xtal=` 이 `?` 이면 측정 실패(회로도 값 40 사용), `19.x~20.x` 면 20MHz 보드입니다.

- `CANA SCAN` — 차가 깨어 있을 때(문 열림·충전·주행) 125k→250k→500k→1M 을 1초씩 들어보고 어느 속도에서 `efmsg`(에러 없는 프레임) 가 잡히는지 보여 줍니다. B 는 그동안 계속 기록됩니다.
- `CANA RATE <bps>` — A 속도를 바꿔 저장하고 바로 다시 엽니다 (`500000` 또는 `500`). 인자 없이 치면 현재 값.
